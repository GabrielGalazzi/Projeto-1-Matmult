...
for j in lista_pontos:
    for h in lista_pontos:
        if ((j == h and lista_pontos.index(j) != lista_pontos.index(h))) or ((((j[0] == h[0] + tolerancia and j[1] == h[1] + tolerancia) or (j[0] == h[0] - tolerancia and j[1] == h[1] - tolerancia)) and lista_pontos.index(j) != lista_pontos.index(h))):
            #print('BRUH')
